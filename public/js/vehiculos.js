document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar tooltips de Bootstrap después de cada redibujado de la tabla
    $('#vehiculosTable').on('draw.dt', function () {
        const tooltipTriggerList = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'));
        tooltipTriggerList.forEach(function (tooltipTriggerEl) {
            // Asegurarse de no inicializar dos veces
        ...
        columns: [
            { 
                ...
                render: function(data, type, row){
                    return `<button class="btn btn-link p-0 fw-bold" onclick="verHistorial(${row.id_vehiculo}, '${data}')">${data}</button>`;
                }
            },
            { 
                data: 'tipo',
                render: function(data) {
                    ...
                    const icon = icons[data] || 'fa-question-circle';
                    // El tooltip en el icono ayuda en vistas móviles donde el texto puede ocultarse
                    return `<i class="fas ${icon} vehicle-icon text-muted" data-bs-toggle="tooltip" title="${data}"></i> <span class="d-none d-md-inline text-capitalize">${data}</span>`;
                }
            },
            { data: 'color' },
            { data: 'modelo' },
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    return `
                        <button class="btn-action" data-bs-toggle="tooltip" title="Editar" onclick="editarVehiculo(${row.id_vehiculo})">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn-action" data-bs-toggle="tooltip" title="Eliminar" onclick="eliminarVehiculo(${row.id_vehiculo})">
...
async function eliminarVehiculo(id) {
    Swal.fire({
        title: '¿Estás seguro?',
        text: "Esta acción no se puede deshacer. El vehículo solo se eliminará si no tiene movimientos activos.",
        icon: 'warning',
    ...
    try{
        ...
        if (!j.data || j.data.length === 0){
            ...
            tb.innerHTML = j.data.map(r => {
                ...
                const diff = total > 0 ? pagado - total : 0;
                const estadoPago = total === 0 ? '-' : (diff >= 0 ? `Vuelto: ${fmt(Math.abs(diff))}` : `Falta: ${fmt(Math.abs(diff))}`);
                const badgeEstado = r.estado==='activo' ? 'success' : (diff>=0 && total>0 ? 'primary' : 'secondary'); // 'primary' para finalizado y pagado
