...
window.reimprimirSalida = async function(idMovimiento){
    try{
        ...
        const doc = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Reimpresión</title>
            ...
            <script>(function(){ try{ new QRCode(document.getElementById('qrcode'), {text:'https://lenixpantoja.com/', width:96, height:96}); }catch(e){} setTimeout(function(){ window.print(); window.close(); }, 400); })();<\/script>
            </body></html>`;
...
function renderLineChart(id, labels, data, label){
    ...
    const primaryColor = getComputedStyle(document.documentElement).getPropertyValue('--primary-color').trim() || '#1a237e';
    if (!ctx) return;
    if (chartIngresos) { chartIngresos.destroy(); }
    chartIngresos = new Chart(ctx, {
        type: 'line',
        data: { labels, datasets: [{ label, data, fill:false, borderColor: primaryColor, tension:0.25 }]},
...
function renderDoughnut(id, labels, data){
    ...
    if (!ctx) return;
    const colors = {
        efectivo: getComputedStyle(document.documentElement).getPropertyValue('--success-color').trim() || '#28a745',
        tarjeta: getComputedStyle(document.documentElement).getPropertyValue('--primary-color').trim() || '#1a237e',
        QR: getComputedStyle(document.documentElement).getPropertyValue('--warning-color').trim() || '#ffc107'
    };
    if (chartMetodo) { chartMetodo.destroy(); }
    chartMetodo = new Chart(ctx, {
        type: 'doughnut',
        data: { labels, datasets: [{ data, backgroundColor: labels.map(l => colors[l] || '#6c757d') }] },
...
async function exportarPDF(){
    try{
        ...
        // --- Rediseño de Encabezado ---
        const headerColor = '#343a40'; // Gris oscuro, más sobrio
        const tableHeaderColor = '#343a40';
        const alternateRowColor = '#f8f9fa'; // Gris muy claro
        ...
        doc.text(`Empresa: ${empresa} | Generado: ${new Date().toLocaleString('es-CO')}`, doc.internal.pageSize.getWidth() - 24, 38, { align: 'right' });
        ...
        doc.autoTable({
            ...
            headStyles: { fillColor: tableHeaderColor, textColor: 255, fontStyle:'bold', halign:'center' },
