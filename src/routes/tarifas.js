...
router.put('/', verifyToken, async (req, res) => {
    try {
        ...
        if (modo_cobro === 'minuto') {
            ...
            valor_hora = 0; valor_hora_adicional = 0; valor_dia_completo = 0; paso_minutos_a_horas = 0; paso_horas_a_dias = 0;
        } else if (modo_cobro === 'hora') {
            if (valor_hora <= 0) return res.status(400).json({success:false,message:'Debe definir valor por hora > 0'});
            valor_minuto = 0; valor_hora_adicional = 0; valor_dia_completo = 0; paso_minutos_a_horas = 0; paso_horas_a_dias = 0;
        } else if (modo_cobro === 'hora_adicional') {
            if (valor_hora <= 0 || valor_hora_adicional <= 0) return res.status(400).json({success:false,message:'Debe definir valor hora y hora adicional > 0'});
            ...
            valor_minuto = 0; valor_hora = 0; valor_hora_adicional = 0; paso_minutos_a_horas = 0; paso_horas_a_dias = 0;
        } else { // mixto
        ...
        try {
            ...
            const [ins] = await conn.query(
                `INSERT INTO tarifas (
                    id_empresa, tipo_vehiculo, valor_hora, valor_minuto, valor_dia_completo, valor_hora_adicional,
