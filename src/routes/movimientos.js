...
function calcularTotal({ minutos, tarifa }) {
    const valorMin = Number(tarifa.valor_minuto);
    const valorHor = Number(tarifa.valor_hora);
    const valorHorAdicional = Number(tarifa.valor_hora_adicional);
    const valorDia = Number(tarifa.valor_dia_completo);
    ...
    if (modo === 'hora_adicional') {
        if (minutos < 1) return { total: Number(valorHor.toFixed(2)), dias: 0, horas: 1, minutos: minutos };
        // Cualquier fracción de minuto cuenta como la primera hora
        if (minutos <= 60) return { total: Number(valorHor.toFixed(2)), dias: 0, horas: 1, minutos: minutos };
        // Después de la primera hora, se cobra la base + adicional por cada hora extra (redondeada hacia arriba)
        const horasAdicionales = Math.ceil((minutos - 60) / 60);
        return { total: Number((valorHor + horasAdicionales * valorHorAdicional).toFixed(2)), dias: 0, horas: 1 + horasAdicionales, minutos: minutos };
    }
    if (modo === 'dia') {
...
router.post('/salida', verifyToken, async (req, res) => {
    try {
        ...
        // Si es solo un pre-cálculo, devolver los datos sin modificar la BD
        if (precalculo) {
            ...
            return res.json({ success: true, data: facturaPrecalculo, message: 'Pre-cálculo exitoso' });
        }

        // --- Proceso de confirmación de salida ---
...
// Confirmar salida y registrar pagos (usado por el modal de pago)
router.post('/confirmar-salida', verifyToken, async (req, res) => {
    ...
    try {
        ...
        if (!id_movimiento || !pagos || !Array.isArray(pagos)) {
            return res.status(400).json({ success: false, message: 'Datos incompletos para confirmar la salida.' });
        }

        await connection.beginTransaction();

        // 1. Obtener el movimiento y verificar que pertenece a la empresa y está activo
        const [movs] = await connection.query(
            'SELECT * FROM movimientos WHERE id_movimiento = ? AND id_empresa = ? FOR UPDATE',
        ...
        if (mov.estado !== 'activo') {
            await connection.rollback();
            return res.status(409).json({ success: false, message: 'Este movimiento ya ha sido finalizado.' });
        }

        // 2. Calcular el total a pagar (re-cálculo para seguridad)
        const [vehiculo] = await connection.query('SELECT tipo FROM vehiculos WHERE id_vehiculo = ?', [mov.id_vehiculo]);
        const tarifa = await obtenerTarifaActiva(idEmpresa, vehiculo[0].tipo);
        if (!tarifa) {
            await connection.rollback();
            return res.status(400).json({ success: false, message: 'No se encontró tarifa activa para el vehículo.' });
        }

        const [tiempo] = await connection.query('SELECT TIMESTAMPDIFF(MINUTE, ?, CURRENT_TIMESTAMP) as minutos', [mov.fecha_entrada]);
        const { total } = calcularTotal({ minutos: tiempo[0].minutos, tarifa });

        // 3. Actualizar el movimiento a 'finalizado'
        await connection.query(
            `UPDATE movimientos SET fecha_salida = CURRENT_TIMESTAMP, total_a_pagar = ?, estado = 'finalizado', id_usuario_salida = ? WHERE id_movimiento = ?`,
            [total, idUsuario, id_movimiento]
        );

        // 4. Insertar los pagos
        if (pagos.length > 0) {
            const pagosValues = pagos.map(p => [idEmpresa, id_movimiento, p.metodo_pago, p.monto, idUsuario]);
            await connection.query(
        ...
        res.json({ success: true, message: 'Salida y pagos confirmados exitosamente.', data: { totalConfirmado: total } });

    } catch (error) {
        if (connection) await connection.rollback();
        console.error('Error al confirmar salida:', error);
        res.status(500).json({ success: false, message: 'Error interno al confirmar la salida.' });
    } finally {
